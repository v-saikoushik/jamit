      ...
      >
        ...
        <form onSubmit={handleSubmit} className="bg-jamit-card rounded-2xl p-8 border border-white/5 space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 rounded-full bg-jamit-green text-black font-semibold hover:bg-jamit-green-hover transition-colors disabled:opacity-50"
          >
            {loading ? 'Logging in...' : 'Log in'}
          </button>
          <p className="text-center text-sm text-jamit-muted">
            No account?{' '}
            <Link to="/register" className="text-jamit-green hover:underline">Sign up</Link>
          </p>
          <p className="text-center text-xs text-jamit-muted pt-2">
            Demo: demo@jamit.app / password123
          </p>
        </form>
      </motion.div>
    </div>
